var duplexer = require('duplexer'),
	...
	stampModule = Handlebars.compile(
		"YUI.add('{{moduleName}}', function(Y) {\n\n" +
		"{{{code}}}" +
		"}, '{{version}}', { requires: ['template-base'{{#each dependencies}}, '{{.}}'{{/each}}] });"
	),
...
Precompiler.prototype = {
	...
	// TODO: better name?
	precompile: function(templateModuleData, filePaths) {
		var pipeline;
	...
	// }
	addReviveTemplateBoilerplate: function() {
	...
	generateYuiModuleForTemplates: function(templateModuleData) {
		var dependencies = [this._engineData.moduleName],
			writeStream = through(),
			moduleGeneratorStream;

		if (templateModuleData.dependencies) {
			dependencies.concat(templateModuleData.dependencies);
		}

		moduleGeneratorStream = concat(function(codeBuffer) {
			var yuiModuleFileContents = stampModule({
			...
			writeStream.end(yuiModuleFileContents);
		});

		return duplexer(moduleGeneratorStream, writeStream);
	}
};
