var _ = require('lodash'),
	...
	processShifterModule = require('./process-shifter-module'),
...
function precompileTemplateBuild(templateBuild) {
	return precompile(templateBuild.files, templateBuild.options);
}

function precompileTemplateBuilds(templateBuilds) {
	return Q.all(_.map(templateBuilds, precompileTemplateBuild));
}

function precompileShifterModule(moduleDir) {
	return processShifterModule(moduleDir).then(precompileTemplateBuilds);
}

exports.precompileShifterModule = precompileShifterModule;
